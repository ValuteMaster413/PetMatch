...
class ServiceProvider(AbstractDIContainer):
    ...
    def _get(self, cls: Type[T], skip_search: bool = False) -> T:
        ...
        if cls in self._transient_implementations.keys() and not skip_search:
            concrete_cls = self._transient_implementations.get(cls)
            return self._get(concrete_cls, skip_search=True)
